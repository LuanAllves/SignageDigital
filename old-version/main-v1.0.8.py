...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                               ...
                               QListWidgetItem, QCheckBox, QGridLayout, QStyle, QFileDialog)
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.upload_button = QPushButton("Upload de Mídia") # 'self' para acessar em outros métodos
        ...
        # Conecta o botão de upload ao método
    ...
    def upload_media(self):
        ...
        file_dialog.setFileMode(QFileDialog.ExistingFiles) # Permite selecionar múltiplos arquivos
        
        # Define os filtros para os tipos de arquivos (imagens e vídeos)
        file_dialog.setNameFilters(["Mídias Suportadas (*.jpg *.jpeg *.png *.gif *.mp4 *.avi *.mov)"])
        
        if file_dialog.exec():
            # Obtém os caminhos dos arquivos selecionados
            selected_files = file_dialog.selectedFiles()
            
            for file_path in selected_files:
                file_name = os.path.basename(file_path)
                # Lógica para determinar o tipo de arquivo
