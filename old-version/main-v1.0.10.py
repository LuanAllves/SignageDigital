...
class DatabaseManager:
    ...
    def add_media(self, name, media_type, file_path):
        ...
        self.cursor.execute('''
            INSERT INTO medias (name, type, file_path)
            VALUES (?, ?, ?)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.db_manager = DatabaseManager() # Instancia o gerenciador de banco de dados
        self.setup_ui()
        self.load_media_from_db() # Carrega as mídias salvas ao iniciar
    ...
    def load_media_from_db(self):
        ...
        for media in medias:
            ...
            # O ID é o primeiro item na tupla de dados do banco de dados (media[0])
            name = media[1]
            media_type = media[2]
            schedule_start_date = media[4] or "Não agendado" # Usa a data se existir, senão "Não agendado"
            
            # Formata a label de agendamento
            if schedule_start_date == "Não agendado":
                schedule_text = "Não agendado"
            else:
                schedule_end_date = media[6]
                schedule_start_time = media[5]
                schedule_end_time = media[7]
                schedule_text = f"Início: {schedule_start_date} às {schedule_start_time} <br> Fim: {schedule_end_date} às {schedule_end_time}"
            
            # Adiciona o item à interface
            self.add_media_item(name, media_type, schedule_text)
    ...
    def upload_media(self):
        ...
        if file_dialog.exec():
            ...
            for file_path in selected_files:
                ...
                self.db_manager.add_media(file_name, file_type, file_path)
                # Adiciona o item à interface
