...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                               ...
                               QSpinBox) # Nova importação
from PySide6.QtGui import QPixmap, QColor, QFont, QImage
from PySide6.QtCore import Qt, QSize, Signal, QBuffer, QIODevice
# ================ Fim das Importações ======================
...
class MediaItemWidget(QWidget):
    ...
    def __init__(self, media_id, media_name, media_type, file_path, duration_seconds, scheduled_date, parent=None):
        ...
        # O self.schedule_label agora será atualizado por um novo método
...
class ScheduleDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Campo para a duração (apenas para imagens)
        ...
        # Campos de agendamento existentes
    ...
    def set_media_type(self, media_type):
        """Define o tipo de mídia para mostrar/esconder o campo de duração."""
...
class MainWindow(QMainWindow):
    ...
    def load_media_from_db(self):
        ...
        for media in medias:
            ...
            self.add_media_item(media_id, name, media_type, file_path, duration_seconds, schedule_text)

    def add_media_item(self, media_id, name, type, file_path, duration, date):
        item_widget = MediaItemWidget(media_id, name, type, file_path, duration, date)
    ...
    def upload_media(self):
        ...
        if file_dialog.exec():
            ...
            for file_path in selected_files:
                ...
                if file_extension in [".jpg", ".jpeg", ".png", ".gif"]:
                    file_type = "Imagem"
                    duration_seconds = 5 # Duração padrão para imagens
                elif file_extension in [".mp4", ".avi", ".mov"]:
                    file_type = "Vídeo"
                    cap = cv2.VideoCapture(file_path)
                    if cap.isOpened():
                ...
                self.add_media_item(media_id, file_name, file_type, file_path, duration_seconds, "Não agendado")

    def open_schedule_dialog(self, media_item_widget):
        dialog = ScheduleDialog(self)
        
        # Passa o tipo de mídia para a janela de agendamento
        dialog.set_media_type(media_item_widget.media_type)
        dialog.duration_spinbox.setValue(media_item_widget.duration_seconds)
        ...
        if dialog.exec():
            ...
            # Atualiza a duração no banco de dados se for uma imagem
            if media_item_widget.media_type == "Imagem":
                self.db_manager.update_media_duration(media_item_widget.media_id, schedule_data['duration'])
                media_item_widget.duration_seconds = schedule_data['duration']
